...
cv::Mat ImageResizer::resizeTo664(const std::string& imagePath) {
    cv::Mat img = cv::imread(imagePath);
    if (img.empty()) {
        throw std::runtime_error("이미지 로드 실패: " + imagePath);
    }

    return resizeTo664(img);
}

cv::Mat ImageResizer::resizeTo664(const cv::Mat& input) {
    if (input.empty()) {
...
bool ImageResizer::resizeTo664AndSave(const std::string& inputPath, const std::string& outputPath) {
    try {
        cv::Mat resized = resizeTo664(inputPath);
